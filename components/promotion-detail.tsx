...
export function PromotionDetail({ id }: { id: string }) {
  ...
  const duplicate = () => {
    window.localStorage.setItem(draftKey, JSON.stringify({ selectedProducts: promotion.products.map((item) => item.productId), channelIds: promotion.channelIds, startDate: promotion.startDate, endDate: promotion.endDate, mechanic: promotion.mechanic, mechanicId: promotion.mechanicId, mechanicConfiguration: promotion.mechanicConfiguration, volumeMode: 'reference', globalVolume: volume, referenceVolumes: Object.fromEntries(promotion.products.map((item) => [item.productId, item.forecastVolume])), economicsMode: 'global', referenceEconomics: {}, usualPrice: promotion.usualPrice, promoPrice: promotion.promoPrice, costPrice: promotion.costPrice ?? 0, promotionalUnitCost: 0, baseQuantity: 200, quantityUnit: 'g', expenses, expenseMode: promotion.expenses.length > 1 ? 'detailed' : 'global', expenseDetails: Object.fromEntries(promotion.expenses.map((expense) => [expense.label, expense.amount])), customExpenses: [], minimumMarginRate: promotion.minimumMarginRate ?? 15, name: `${promotion.name} — copie` }));
