...
export function PromotionWizard() {
  ...
  function compareScenarios() {
    ...
    setIsComparing(true);
    window.setTimeout(() => { setIsComparing(false); setShowScenarios(true); }, 550);
  }
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    ...
    <Card className="min-h-[430px] p-6 md:p-8">
      ...
      {step === 4 && <section><h2 className="text-2xl font-bold">Économie de la promotion</h2><p className="mt-2 text-navy-500">Les résultats se recalculent immédiatement.</p><div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><Field label="Prix habituel"><input className={inputClass} min="0" step="0.01" type="number" onClick={(event) => { if (event.currentTarget.value === '0') event.currentTarget.select(); }} onFocus={(event) => event.currentTarget.select()} value={draft.usualPrice} onChange={(event) => update('usualPrice', Number(event.target.value))} /></Field><Field label="Prix promotionnel"><input className={inputClass} min="0" step="0.01" type="number" onClick={(event) => { if (event.currentTarget.value === '0') event.currentTarget.select(); }} onFocus={(event) => event.currentTarget.select()} value={draft.promoPrice} onChange={(event) => update('promoPrice', Number(event.target.value))} /></Field><Field label="PRI / coût de revient" hint="Coût unitaire retenu pour estimer la marge brute."><input className={inputClass} min="0" step="0.01" type="number" onClick={(event) => { if (event.currentTarget.value === '0') event.currentTarget.select(); }} onFocus={(event) => event.currentTarget.select()} value={draft.costPrice} onChange={(event) => update('costPrice', Number(event.target.value))} /></Field><Field label="Volume prévisionnel"><input className={inputClass} min="0" type="number" onClick={(event) => { if (event.currentTarget.value === '0') event.currentTarget.select(); }} onFocus={(event) => event.currentTarget.select()} value={totalVolume} onChange={(event) => { update('volumeMode', 'global'); update('globalVolume', Number(event.target.value)); }} /></Field><Field label="Dépenses liées à l’opération"><input className={inputClass} disabled={showExpenses} min="0" type="number" onClick={(event) => { if (event.currentTarget.value === '0') event.currentTarget.select(); }} onFocus={(event) => event.currentTarget.select()} value={expenses} onChange={(event) => update('expenses', Number(event.target.value))} /></Field><Field label="Objectif minimum de marge"><input className={inputClass} min="0" step="0.1" type="number" onClick={(event) => { if (event.currentTarget.value === '0') event.currentTarget.select(); }} onFocus={(event) => event.currentTarget.select()} value={draft.minimumMarginRate} onChange={(event) => update('minimumMarginRate', Number(event.target.value))} /></Field></div><div className="mt-3 flex items-center justify-between"><p className="font-bold text-mint-700">Remise : -{metrics.discountRate.toFixed(1)} %</p><Button variant="ghost" onClick={() => setShowExpenses(!showExpenses)}>{showExpenses ? 'Masquer le détail' : '+ Détailler les dépenses'}</Button></div>{showExpenses && <div className="mt-4 grid gap-3 rounded-2xl bg-navy-50 p-4 sm:grid-cols-2">{(getChannel(draft.channelIds[0])?.type === 'GMS' ? expenseLabels.GMS : expenseLabels.ecommerce).map((label) => <Field label={label} key={label}><input className={inputClass} min="0" type="number" onClick={(event) => { if (event.currentTarget.value === '0') event.currentTarget.select(); }} onFocus={(event) => event.currentTarget.select()} value={draft.expenseDetails[label] || 0} onChange={(event) => update('expenseDetails', { ...draft.expenseDetails, [label]: Number(event.target.value) })} /></Field>)}<p className="font-bold sm:col-span-2">Total détaillé : {euro(detailedExpenses)}</p></div>}<div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[['CA prévisionnel', euro(metrics.revenue)], ['Marge après dépenses', euro(metrics.marginAfterExpenses)], ['Taux de marge', `${metrics.marginRate.toFixed(1)} %`], ['ROI promotionnel', `${metrics.roi.toFixed(1)}×`]].map(([label, value]) => <div className="rounded-2xl border border-navy-100 p-4" key={label}><p className="text-xs font-semibold text-navy-500">{label}</p><p className="mt-2 text-xl font-black">{value}</p></div>)}</div></section>}
      {step === 5 && <section><h2 className="text-2xl font-bold">Analyse de votre promotion</h2><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[['CA prévisionnel', euro(metrics.revenue)], ['Marge', `${metrics.marginRate.toFixed(1)} %`], ['ROI', `${metrics.roi.toFixed(1)}×`], ['Volume', totalVolume.toLocaleString('fr-FR')]].map(([label, value]) => <div className="rounded-2xl bg-navy-50 p-4" key={label}><p className="text-sm text-navy-500">{label}</p><p className="mt-2 text-2xl font-black">{value}</p></div>)}</div><div className="mt-7 grid gap-4 md:grid-cols-3"><Control title="Réglementation" status={compliance[0].status} text={compliance[0].explanation} /><Control title="Rentabilité" status={metrics.marginRate >= draft.minimumMarginRate ? 'Conforme' : 'Vigilance'} text={metrics.marginRate >= draft.minimumMarginRate ? 'L’objectif de marge saisi est atteint.' : `Marge ${metrics.marginRate.toFixed(1)} % sous l’objectif de ${draft.minimumMarginRate} %.`} /><Control title="Cohérence / opérationnel" status={overlaps.length ? 'Vigilance' : 'Conforme'} text={overlaps.length ? 'Un chevauchement doit être vérifié.' : 'Dates et données essentielles cohérentes.'} /></div>{metrics.marginRate < draft.minimumMarginRate && <div className="mt-6 rounded-2xl border border-mint-200 bg-mint-50 p-5"><Badge tone="success">Recommandation déterministe</Badge><div className="mt-4 grid gap-4 md:grid-cols-2"><div><p className="text-sm text-navy-500">Votre scénario</p><p className="mt-1 font-bold">Remise -{metrics.discountRate.toFixed(1)} % · Marge {metrics.marginRate.toFixed(1)} %</p></div><div><p className="text-sm text-navy-500">Alternative proposée</p><p className="mt-1 font-bold">Passer à -{scenarios[1].metrics.discountRate.toFixed(1)} % · Marge {scenarios[1].metrics.marginRate.toFixed(1)} % · ROI {scenarios[1].metrics.roi.toFixed(1)}×</p></div></div><Button className="mt-4" variant="secondary" disabled={isComparing} onClick={compareScenarios}>{isComparing ? <PulseLoader label="Comparaison en cours" /> : showScenarios ? "Masquer les scénarios" : "Comparer les scénarios"}</Button></div>}{showScenarios && <ScenarioTable scenarios={scenarios} />}</section>}
